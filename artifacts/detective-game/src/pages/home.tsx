...
import { Star, MapPin, Lock, ChevronRight, Trophy, AlertCircle } from "lucide-react";
      ...
      <div className="mb-8">
        <div className="flex items-start justify-between">
          <div>
            ...
            <h1 className="font-cinzel text-3xl font-black text-foreground">
              القضايا المفتوحة
            </h1>
          </div>
          {player && (
          ...
          {cases?.map((c) => {
            ...
            return (
              <div
                ...
                className="suspect-card relative bg-card border border-border rounded-xl overflow-hidden group cursor-pointer"
                onClick={() => {
                  if (c.isPremium && !player?.hasPaid && !player?.paymentExempt) {
                    setLocation("/premium");
                  } else {
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    {c.isPremium && (
                      <span className="ml-auto flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-primary/15 border border-primary/30 text-primary font-medium">
                        <Lock className="w-2.5 h-2.5" />
                        مميز
                      </span>
                    )}
                    {isSolved && (
                      <span className="ml-auto text-xs px-2 py-0.5 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 font-medium">
                        محلولة
                      </span>
                    )}
                  </div>
                ...
                {c.isPremium && !player?.hasPaid && !player?.paymentExempt && (
